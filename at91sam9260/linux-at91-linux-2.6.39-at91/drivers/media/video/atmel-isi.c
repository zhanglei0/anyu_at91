			...
			u32 height, enum v4l2_mbus_pixelcode code)
{
	...
	cfg1 = isi_readl(isi,ISI_CR1);
	isi_writel(isi, ISI_CR1, ISI_CTRL_DIS);

	cfg2 = isi_readl(isi, ISI_CR2);
	cfg2 |= cr;
	/* Set width */
	...
	isi_writel(isi, ISI_CR2, cfg2);
	isi_writel(isi,ISI_CR1,cfg1);
	return 0;
}

static irqreturn_t atmel_isi_handle_streaming(struct atmel_isi *isi)
{
	if (isi->active && isi->active->magic == ISI_MAGIC) {
	...
	if (list_empty(&isi->video_buffer_list)) {
		...
		isi->active = list_entry(isi->video_buffer_list.prev,   //prev or next?
					struct frame_buffer, list);
		if(isi != NULL && isi->active->magic == ISI_MAGIC) {
			isi_writel(isi,ISI_PPFBD,isi->active->p_dma_desc->fbd_phys);
		}
		else {
			printk("active 1 format is wrong\n");
		}
	}
	return IRQ_HANDLED;
}
...
{
	struct atmel_isi *isi = (struct atmel_isi *)dev_id;
	u32 status, mask, pending;
	irqreturn_t ret = IRQ_NONE;
	if(!isi || (isi->magic != ISI_MAGIC)) {
		printk("isi_interrupt wrong format\n");
		return ret;
	}
	spin_lock(&isi->lock);
	status = isi_readl(isi, ISI_SR);
	mask = isi_readl(isi, ISI_IMR);
	pending = status & mask;
	if (pending & ISI_SR_SRST) {
		isi_writel(isi, ISI_IDR, ISI_CTRL_SRST);
		ret = IRQ_HANDLED;
	} else if (pending & ISI_CTRL_DIS) {
		isi_writel(isi, ISI_IDR, ISI_CTRL_DIS);
		ret = IRQ_HANDLED;
	} else {
		if (pending & (ISI_SR_SOF)) {
			ret = atmel_isi_handle_streaming(isi);
		}
		else 
			printk("isi_interrupt %04x, %04x\n",status,mask);
	}
...
static int atmel_isi_wait_status(struct atmel_isi *isi, int wait_reset)
{
	...
	if (wait_reset) {
		isi_writel(isi, ISI_CR1, ISI_CTRL_SRST);
	} else {
		isi_writel(isi, ISI_CR1, ISI_CTRL_DIS);
	}
				...
				void *alloc_ctxs[])
{
	...
	u32 cfg1 = isi_readl(isi, ISI_CR1);
	cfg1 &= ~(ISI_CTRL_DIS);
	isi_writel(isi,ISI_CR1,cfg1);
	/* Reset ISI */
...
static int buffer_prepare(struct vb2_buffer *vb)
{
	...
	if (!buf->p_dma_desc) {
		if (list_empty(&isi->dma_desc_head)) {
			...
			desc->p_fbd->next_fbd_address = desc->fbd_phys;       // 0?
			buf->p_dma_desc = desc;
		}
	}
	return 0;
...
static void start_dma(struct atmel_isi *isi, struct frame_buffer *buffer)
{
	...
	isi_writel(isi, ISI_IER,ISI_IER_ENABLE_SOF);

	/* Check if already in a frame */
	if (isi_readl(isi, ISI_SR) & ISI_CTRL_CDC) {
		dev_err(isi->icd->dev.parent, "Already in frame handling.\n");
		return;
	}
	if(buffer->p_dma_desc->fbd_phys < (PHYS_OFFSET + (64UL <<20)) &&
	   buffer->p_dma_desc->fbd_phys >= PHYS_OFFSET) {
		isi_writel(isi,ISI_PPFBD,buffer->p_dma_desc->fbd_phys);
		isi_writel(isi,ISI_PSIZE,(479UL << 16) | (639));
		isi_writel(isi,ISI_PDECF,18);
		/* Enable linked list */
		cfg1 |= isi->pdata->frate;
		isi_writel(isi, ISI_CR1, cfg1);
	}
	else {
		printk("fbd_phys fault:%08X\n",buffer->p_dma_desc->fbd_phys);
		isi_writel(isi, ISI_CR1, ISI_CTRL_DIS);
	}
}
...
static int start_streaming(struct vb2_queue *vq)
{
	...
	sr = isi_readl(isi, ISI_SR);
	/* Enable CODEC interrupt */
	isi_writel(isi, ISI_IDR, ISI_IER_ENABLE_SOF);
...
static int stop_streaming(struct vb2_queue *vq)
{
	...
	while ((isi_readl(isi, ISI_SR) & ISI_CTRL_CDC) &&
