...
static int soc_camera_open(struct file *file)
{
	...
	if (icd->use_count == 1) {
		...
		if (ret < 0) {
			printk("soc_camera_power_set failewd %d\n",ret);
			goto epower;
		}
		...
		if (ret < 0) {
			...
			printk("Couldn't activate the camera: %d\n", ret);
			goto eiciadd;
		}
		...
		if (ret < 0) {
			printk("soc_camera_set_fmt failed %d\n",ret);
			goto esfmt;
		}
		...
		if (ici->ops->init_videobuf) {
			...
			if (ret < 0) {
				printk("init_videobuf2 failed %d\n",ret);
				goto einitvb;
			}
		}
	}
			       ...
			       enum v4l2_buf_type i)
{
	...
	if (!ret) {
		printk("v4l2_subdev_call:s_stream\n");
...
static int soc_camera_probe(struct device *dev)
{
	...
	if (icl->board_info) {
		...
		if (ret < 0) {
			printk("soc_camera_init_i2c failed %d\n",ret);
			goto eadddev;
		}
	...
	if (ret < 0) {
		printk("soc_camera_init_user_formats failed %d\n",ret);
		goto eiufmt;
	}
...
static int __devinit soc_camera_pdrv_probe(struct platform_device *pdev)
{
	...
	printk("soc_camera_pdrv_probe Success\n");
	return 0;
